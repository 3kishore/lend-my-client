...
export class ApplicationStatusComponent {
  ...
  @ViewChild('orderCountTemplate', {static: true})
  public orderCountTemplate: TemplateRef<any>;
  ...
  ngOnInit(): void {
    this.gridConfig = this.configService.initializeGidConfig(this.actionTemplate, this.customerNameTemplate, this.orderCountTemplate);
    this.gridActionData = {
      filters: [],
      sort: {
        sortKey: '',
  ...
  getLoanStatus() {
    ...
    this.component$.add(
      this.configService.getLoanStatus(this.gridActionData, {}, {}).subscribe({
        next: (resp: any) =>  {
